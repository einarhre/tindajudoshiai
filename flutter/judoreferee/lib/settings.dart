...
Future<void> readSettings() async {
  ...
  numTatamis = await getValInt('numtatamis', 3);
  refsPerTatami = await getValInt('refspertatami', 5);
}
...
final GlobalKey<FormState> _numTatamisKey = GlobalKey<FormState>();
final GlobalKey<FormState> _refsPerTatamiKey = GlobalKey<FormState>();
    ...
    lst = lst +
        [
          ...
          CardSettingsInt(
              key: _numTatamisKey,
              label: 'Tatamis',
              initialValue: numTatamis,
              onSaved: (value) {
                setState(() {
                  numTatamis = value ?? 3;
                });
                setValInt('numtatamis', numTatamis);
              },
          ...
          CardSettingsInt(
              key: _refsPerTatamiKey,
              label: 'Referees per tatami',
              initialValue: refsPerTatami,
              autovalidateMode: _autoValidateMode,
              validator: (value) {
                if (value != null) {
                  if (value > 6) return 'Number must be 1 - 6';
                  if (value < 1) return 'Number must be 1 - 6';
                }
                return null;
              }),
        ];
